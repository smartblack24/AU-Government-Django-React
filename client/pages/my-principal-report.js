...
const getMatters = gql`
  query myMatterReportMatter(
    $after: String, $principalName: String,
    $billableStatus: String, $leadType: Boolean
  ) {
    matters(
      first: 15, after: $after, principalName: $principalName,
...
class MatterReport extends React.Component {
  ...
  handleOnAutocompleteEdit = () => {
    this.setState({ principalName: null })
  }
  handleOnStaffSelect = ({ staff }) => {
    this.setState({ principalName: staff, billableStatus: 1 })
  ...
  handleLeadTypeChange = (event) => {
    ...
    this.props.data.refetch({
      ...
      principalName: this.state.principalName.fullName,
      leadType: type,
    })
  ...
  renderPdfButton = () => (
    <a
      href={`${BACKEND_URL}/pdf/principal-report/${this.state.principalName && this.state.principalName.id}/${
  ...
  render() {
    ...
    if (data.loading) {
      return (
        <Page pageTitle="My Principal report" user={user}>
          <LoadSpinner />
        </Page>
      )
    }
    ...
    return (
      <Page user={user} pageTitle="My Principal report" renderRight={this.renderPdfButton()}>
  ...
  withAuth(
    compose(
      ...
      graphql(getMatters, { options: ({ user }) => (
        { variables: { billableStatus: 1, principalName: user.fullName } }) }),
    )(MatterReport),
  ),
)
